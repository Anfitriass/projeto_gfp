...
export default function CategoriasModal({ modalAberto, fecharModal, itemAlterar }) {
    ...
    const [cor, setCor] = useState(listaCores[10]);
            ...
            <section className="w-full max-w-2xl bg-white p-8 rounded-lg shadow-lg text-gray-800" >
                ...
                <div className="space-y-5">
                    <div className="flex rounded-md shadow-sm" >
                        <button type="button" onClick={() => setTipoTransacao('ENTRADA')} className={`flex-1 p-2 rounded-l-md ${tipoTransacao == 'ENTRADA' ? 'bg-cyan-500 text-white' : 'bg-gray-200'}`} >ENTRADA</button>
                        <button type="button" onClick={() => setTipoTransacao('SAIDA')} className={`flex-1 p-2 rounded-r-md ${tipoTransacao == 'SAIDA' ? 'bg-orange-500 text-white' : 'bg-gray-200'}`} >SAIDA</button>
                    </div>

                    <div className="flex items-center gap-3" >
                        <div className="w-2/3" >
                            <label className={Estilos.labelCadastro} >Nome da categoria</label>
                            <input type="text" value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Ex.: Alimentacao" className={Estilos.inputCadastro} />
                        </div>
                        <input type="checkbox" checked={gastoFixo} onChange={(e) => setGastoFixo(e.target.checked)} />
                        <label>Gasto Fixo?</label>
                    </div>

                    <label className={Estilos.labelCadastro} >Descrição de categoria (Opcional)</label>
                    <input type="text" value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Ex.: Gasto com supermercado" className={Estilos.inputCadastro} />
                    ...
                    <div className={Estilos.listaItensModalCategorias} >
                        {
                            listaCores.map((corItem) => (
                                <div key={corItem} onClick={() => setCor(corItem)} style={{ backgroundColor: corItem }} className={`${Estilos.iconeCorBotaoModal} ${cor == corItem ? 'ring-2 ring-offset-2 ring-cyan-500' : ''}`} />
                            ))
                        }
                    </div>
                    ...
                    <div className={Estilos.listaItensModalCategorias} >
                        {
                            listaIcones.map((iconeItem) => (
                                <div key={iconeItem} onClick={() => setIcone(iconeItem)} style={{ backgroundColor: cor }} className={`${Estilos.iconeCorBotaoModal} ${icone == iconeItem ? 'ring-2 ring-offset-2 ring-cyan-500' : ''}`}
                                >
