        ...
        <div className="flex justify-center py-6 px-4" >
            <section className="w-full max-w-lg bg-white p-8 rounded-lg shadow-lg text-gray-800" >
                ...
                <div className="space-y-5">
                    ...
                    <input type="text" value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Ex.: Carteira" className={Estilos.inputCadastro} />
                    <label className={Estilos.labelCadastro} >Tipo de conta</label>
                    <select value={tipoConta} onChange={(e) => setTipoConta(e.target.value)} className={Estilos.inputCadastro} >
                        <option value="CONTA_CORRENTE">Conta Corrente</option>
                        <option value="POUPANCA">Poupança</option>
                        <option value="CARTAO_CREDITO">Cartão Crédito</option>
                        <option value="CARTAO_DEBITO">Cartão Bébito</option>
                        <option value="DINHEIRO">Dinheiro</option>
                        <option value="INVESTIMENTO">Investimento</option>
                    </select>
                    <label className={Estilos.labelCadastro} >Saldo inicial</label>
                    <input type="number" value={saldoInicial} onChange={(e) => setSaldoInicial(e.target.value)} placeholder="Ex.: Carteira" className={Estilos.inputCadastro} />
                    {/* Botões de controle */}
